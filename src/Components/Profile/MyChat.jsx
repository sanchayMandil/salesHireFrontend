...
function MyChat({ setOption, type }) {
 ...
 const listOfChat = [
  ...
  const handleChatButtonClick = () => {
    if (setOption) {
      setOption(0); // Switch to ProfileContainer within Profile
    } else {
      nav(route); // Navigate to the route specified by type (e.g., /home)
    }
  };

  return (
    <div
      className={`relative bg-gradient-to-br from-[#393939] to-[#121212] ${
        type ? "w-[100vw]" : "w-[85vw]"
      } h-[100vh] grid grid-cols-2 max-md:grid-cols-1 max-md:w-[100vw]`}
    >
      <div
        className={`border-r-[0.2vw] border-gray-700 overflow-y-auto max-md:${
          show ? "hidden" : "block"
        }`}
      >
        {/* Chat title */}
...
Menu icon"
            ...
            {open && (
              <div className="fixed top-1/2 left-[45vw] transform -translate-x-1/2 -translate-y-1/2 w-[25vw] bg-[#444444] shadow-md rounded-[2vw] z-50 text-[2vw] overflow-hidden text-white px-[1vw]">
                <ul className="py-2">
                  <li className="px-4 py-2 cursor-pointer">Support & Help</li>
                  <li className="px

-4 py-2 cursor-pointer">Manage Block Contact</li>
                </ul>
              </div>
            )}
            </div>
          ...
          <div className="flex justify-between items-center pl-[2vw] pr-[1vw] bg-gradient-to-r from-[#4a4a4a] to-[#444444] rounded-l-full rounded-r-full py-[0.5vw] mx-[1vw] h-[3.5vw] max-md:h-[10vw] max-md:pl-[5vw] max-md:pr-[3vw]">
            <div className="text-[1vw] max-md:text-[3vw]">
              ...
              />
              <input
                className="pl-[1vw] focus:outline-none placeholder:text-white/50"
                type="text"
            ...
            <button
              type="button"
              className="bg-blue-700 p-[0.7vw] rounded-full cursor-pointer"
            >
              <img
          ...
          <div className="flex flex-col justify-start max-md:mt-[2vw]">
            {listOfChat.map((values) => (
              <button
                ...
                onClick={() => {
                  setUserId(values.id);
                  setName(values.name); // Fixed: setNAme → setName
                  setPic(values.image);
                  setShow(true);
                }}
                type="button"
                className="flex justify-start items-center p-[1vw] cursor-pointer hover:bg-gray-800"
              >
                <img
                  className="rounded-full h-[5vw] object-center max-md:h-[15vw]"
                  src={values.image}
                  alt=""
                />
                <div className="flex flex-col justify-start items-start text-white pl-[1vw]">
                  <h1 className="text-[1.3vw] max-md:text-[5vw]">
                    {values.name}
                  </h1>
                  <p className="text-[0.8vw] text-white/50 max-md:text-[2vw]">
                    {values.lastMessage}
                  </p>
                  <span className="text-[0.7vw] text-white/50 max-md:text-[1.5vw]">
                    {values.time}
                  </span>
                </div>
              </button>
        ...
        {/* Conditionally render ChatBox only when a chat is selected */}
        {show && <ChatBox id={userId} name={name} img={pic} />}
      </div>
    );
}
